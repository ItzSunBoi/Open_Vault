...
from open_vault.multiplayer.parties import PartyService
from open_vault.games import connect4, uno, codenames, mafia, curvefever
...
PAGES = [
    ...
    'connect4', 'uno', 'codenames', 'mafia', 'curvefever',
]
...
PAGE_ALIASES = {
    ...
    'vault-wing': 'flapty',
    'vault-neon': 'neonrun',
    'vault-connect4': 'connect4',
    'vault-uno': 'uno',
    'vault-codenames': 'codenames',
    'vault-mafia': 'mafia',
    'vault-curve': 'curvefever',
}
...
MULTIPLAYER_GAMES = ['drawing','wordgame','chess','pong','battleship','trivia','bomberman','connect4','uno','codenames','mafia','curvefever']


def create_party(party_id):
    return get_party_service().create_party(party_id)
...
def remove_client_from_party(client_id, party_id=None):
    return get_party_service().remove_client(client_id, party_id)


def get_party_stats(party_id):
    return get_party_service().get_party_stats(party_id, MULTIPLAYER_GAMES)


def get_party_payload(party_id):
    return get_party_service().get_payload(party_id, MULTIPLAYER_GAMES)


def broadcast_party_state(party_id):
    get_party_service().broadcast_party_state(party_id, MULTIPLAYER_GAMES)


def join_party(client_id, party_id, name):
    party_key = get_party_service().join_party(client_id, party_id, name)
    if party_key:
...
def sanitize_game_name(game):
    ...
    return cleaned if cleaned in ['drawing','wordgame','chess','pong','battleship','trivia','bomberman','connect4','uno','codenames','mafia','curvefever'] else None

_party_service = None

def get_party_service():
    global _party_service
    if _party_service is None:
        _party_service = PartyService(
            parties=parties,
            clients=clients,
            sanitize_room_id=sanitize_room_id,
            sanitize_player_name=sanitize_player_name,
            ws_send=ws_send,
        )
...
def sync_client_to_room(client_id, room, player):
    ...
    elif game == 'connect4' and room.get('c4', {}).get('running'):
        ws_send(client_id, {'type': 'c4_state', 'state': connect4.public_state(room)})
    elif game == 'uno' and room.get('uno', {}).get('running'):
        uno.handle(client_id, room, {'type': 'noop'}, ws_send)
    elif game == 'codenames' and room.get('codenames', {}).get('running'):
        codenames.handle(client_id, room, {'type': 'noop'}, ws_send)
    elif game == 'mafia' and room.get('mafia', {}).get('running'):
        mafia.handle(client_id, room, {'type': 'noop'}, ws_send)
    elif game == 'curvefever' and room.get('curvefever', {}).get('running'):
        ws_send(client_id, {'type': 'curve_state', **room.get('curvefever', {})})
...
def create_room(room_key, game, public_room_id=None):
    return {
        ...
        'c4': {'running': False},
        'uno': {'running': False},
        'codenames': {'running': False},
        'mafia': {'running': False},
        'curvefever': {'running': False},
    }
...
def leave_room(client_id):
    ...
    elif room['game'] in ('connect4', 'uno', 'codenames', 'mafia', 'curvefever') and len(room['players']) <= 1:
        room[{'connect4': 'c4', 'uno': 'uno', 'codenames': 'codenames', 'mafia': 'mafia', 'curvefever': 'curvefever'}[room['game']]]['running'] = False
...
def handle_message(client_id, msg, cookie_profile=None):
    ...
    elif game == 'connect4':   connect4.handle(client_id, room, msg, ws_send)
    elif game == 'uno':        uno.handle(client_id, room, msg, ws_send)
    elif game == 'codenames':  codenames.handle(client_id, room, msg, ws_send)
    elif game == 'mafia':      mafia.handle(client_id, room, msg, ws_send)
    elif game == 'curvefever': curvefever.handle(client_id, room, msg, ws_send)

    party_id = info.get('party')
    leader_ok = True
    if party_id and party_id in parties:
        leader_ok = get_party_service().is_leader(party_id, client_id)

    if t == 'start_game' and room['host'] == client_id and leader_ok:
        ...
        elif game == 'connect4':  connect4.start(room, ws_send)
        elif game == 'uno':       uno.start(room, ws_send)
        elif game == 'codenames': codenames.start(room, ws_send)
        elif game == 'mafia':     mafia.start(room, ws_send)
        elif game == 'curvefever': curvefever.start(room, ws_send)
